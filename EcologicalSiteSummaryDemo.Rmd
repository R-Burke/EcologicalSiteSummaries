...
![Photo by Patrick Alexander](Playa.jpg)
...
## Getting set up {.tabset .tabset-fade .tabset-pills}

###
### I want this report generated for me 

If you have R limitations and would prefer the report to be generated for you, use this [template](https://github.com/R-Burke/EcologicalSiteSummaries/blob/master/ESS_Template.xlsx) to request that Jornada run the report. 
...
The script to generate this report is available [here](https://github.com/R-Burke/EcologicalSiteSummaries/blob/master/EcologicalSiteSummaryReport_09032019.Rmd). This script has file pathways omitted for security issues. You will need to copy and paste the appropriate file paths at the beginning of the script. 
 
**BLM Users:** make sure you have R installed on your local drive and that packages are being installed on your local drive as well. You most likely will not be able to render this report on a network drive. *You will need to follow the step by step instructions in the R and SQL set up tutorial to generate this report. It's a lot of steps at the beginning, but will eventually allow you to generate rapid automated reporting (once set up, this report should take less than 5 minutes per ecological site)*.  

####

#### Prepping the Data {.tabset .tabset-fade .tabset-pills}

To generate these reports, a few things need to be specified at the beginning of this script. These include your state and your ecological site(s) of interest. The script is automatically directed to the TerrADat database via the SQL connection you set up.  From here on, the reports is automated. 
*Note: these reports only work for regions with a state species list.*

##### Install and Load Packages

You will need to install several packages to run this report. Make sure every package in library() is installed in your library folder within your R folder on your local drive (i.e. C:/Users/JSmith/Documents/R/win-library). You may have to do this manually on a BLM computer rather than a batch installation (Tools -> Install Packages).  

``` {r ESS Demo, echo = TRUE, include = TRUE}
...
##### Define your ecological site(s) and state 

*Note: you do not need to specify your state if you want a report across state lines.*
...
#The rest of this report is automated. You can now run the report. 
#When you click knit to html, save as a .Rmd file. 

```

##### Connect to AIM Pub

*This is only for visualization of the database connection. This section requires no changes.*

```{r pub connection , echo = TRUE , include = TRUE}
...
# 2. Where is this data coming from? {.tabset .tabset-fade .tabset-pills}
##
## Plot Map \
...
Year <- TDat_LMF_plots_Year$DateEstablished
...
Map <- addTiles(Map) %>% addCircleMarkers(lng = ~Longitude_NAD83 , lat = ~Latitude_NAD83 , radius = 3 , 
                                          ...
                                          color = ~Pal_Date(DateEstablished) , 
                                          ...
                                          addLegend(pal = Pal_Date , values = TDat_LMF_plots_Year$DateEstablished , opacity = 1 , group = Year) %>%
...
# 3. Interpreting report figures {.tabset .tabset-fade .tabset-pills}
...
Plots_Simple <- TDat_LMF_plots_Year %>% dplyr::select(PrimaryKey , DateEstablished, EcologicalSiteId) %>% mutate(Plots = Plots) %>%
                arrange(Year)
...
PlotsPerYear_Total <- ggplot(Plots_Simple , aes(Plots)) +
                geom_bar(stat = "count" , position = position_stack(reverse = TRUE) , 
                         aes(fill = DateEstablished) , width = .2 , ) +
...
PlotsPerYear_EcoSite <- ggplot(Plots_Simple_EcoSite , aes(Plots)) +
                geom_bar(stat = "count" , position = position_stack(reverse = TRUE) , 
                         aes(fill = DateEstablished , text = EcologicalSiteId) , width = .2 , ) +
                scale_fill_manual(values = Year_Palette) +
                coord_flip() + theme(axis.text.y = element_blank())
