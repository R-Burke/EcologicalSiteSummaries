...
Download the RMarkdown script and supporting files by downloading the full Ecological Site Summary repository in a zipped folder [here](https://github.com/R-Burke/EcologicalSiteSummaries/blob/master/ESS_BLM.Rmd). Unzip the folder into your local drive documents (i.e. C:/Users/JSmith/Documents/EcologicalSiteSummaries). Open the ESS_BLM.Rmd file that's in the repository you unzipped to your local drive. Open the file using RStudio. The supporting documents in this folder will be necessary to run the script. This is an RMarkdown script with an HTML output. 
...
To generate these reports, you need to specify your ecological site(s) of interest. You can specify a single ecological site that is restricted by state lines or create a list of multiple ecological sites that can span political boundaries. In the "" after params: Ecosite: "" in the heading of this document, type in your ecological site of interest. This tool works best with ecological sites that have a large sample size. 

```{r file paths , echo = TRUE , include = TRUE}

#Define your ecological site

EcoSite <- params$EcoSite

```

##### Connect to the database

The script on GitHub has file pathways omitted for security issues. You will need to copy and paste the appropriate file paths to set up your SQL connection below line 116 if you are internal on a BLM server (get these paths from page 15 of the R SQL for BLM users tutorial). 
...
EcoSiteList <- TDat_LMF$EcologicalSiteId
...
TDat_LMF_plots_Year <- TDat_LMF %>% 
                    arrange(EcologicalSiteId) #Sorting this for the map and remove 0,0 lat lon
...
EcoSitePlots <- TDat_LMF %>% filter(EcologicalSiteId == params$EcoSite)
...
SubShrub_Species_Ly <- ggplotly(Species_Plots_Interactive[["SubShrub.Perennial"]] , tooltip = "text")
SubShrub_Species_Ly
```
...
Shrub_Species_Ly <- ggplotly(Species_Plots_Interactive[["Shrub.Perennial"]] , tooltip = "text")
Shrub_Species_Ly
```
...
Tree_Species_Ly <- ggplotly(Species_Plots_Interactive[["Tree.Perennial"]] , tooltip = "text")
Tree_Species_Ly
```
...
Succulent_Species_Ly <- ggplotly(Species_Plots_Interactive[["Succulent.Perennial"]] , tooltip = "text")
...
NoxNNox_ByPlot <- Merged_Species_PercCover %>% group_by(Noxious , PrimaryKey , PlotID) %>% 
                        filter(!is.na(AH_SpeciesCover)) %>%
                         summarize(Percent = sum(AH_SpeciesCover)) %>%
                         mutate_if(is.numeric, round , digits = 2) 
...
Summary_SG_Group_ByPlot <- Merged_Species_SG %>% filter(!is.na(AH_SpeciesCover)) %>%
                         group_by(SG.Group, PrimaryKey , PlotID) %>%
                         summarize(Percent = sum(AH_SpeciesCover)) %>%
                         mutate_if(is.numeric, round , digits = 2)
...
### Percent Cover, Combined Perennial Herbaceous {.tabset .tabset-fade .tabset-pills}

####
#### By Plot

```{r combined perennial herbaceous plot , echo = FALSE , results = 'asis'}

PGrass_ForbPlot <- Merged_Species_PercCover %>% subset(GrowthHabitSub == "Graminoid" | GrowthHabitSub == "Forb") %>% subset(Duration == "Perennial") %>%
                   group_by(GrowthHabitSub , Duration , PlotID , PrimaryKey) %>% 
                         summarize(Percent = sum(AH_SpeciesCover)) %>%
                         mutate_if(is.numeric, round , digits = 2) 
  
PGrass_ForbPlot %>% DT::datatable(extensions = 'Buttons', filter = "top" , options = list(scrollX = TRUE ,
                          dom = 'Bfrtip',
                          buttons =
                          list(list(
                          extend = 'collection',
...
#### Ecological Site
```{r combined perennial herbaceous site , echo = FALSE , results = 'asis'}

PGrass_Forb <- Merged_Species_PercCover %>% subset(GrowthHabitSub == "Graminoid" | GrowthHabitSub == "Forb") %>% subset(Duration == "Perennial") %>%
...
PGrass_Forb %>% DT::datatable(extensions = 'Buttons', filter = "top" , options = list(scrollX = TRUE ,
                          dom = 'Bfrtip',
                          buttons =
                          list(list(
                          extend = 'collection',
...
HgtPrep_Sagebrush <- HgtPrep %>% subset(SG.Group == "Sagebrush")
...
Heights_Summary_SFG_Plot <- HgtPrep %>% group_by(GrowthHabitSub , Duration , PrimaryKey , PlotID) %>%
                             summarize(AverageHeight_cm = mean(Hgt_Species_Avg) ,
                             Standard_Deviation_cm = sd(Hgt_Species_Avg),
                             MinHeight_cm = min(Hgt_Species_Avg) ,
                             MaxHeight_cm = max(Hgt_Species_Avg))%>%
...
Heights_Summary_PerPlot <- HgtPrep %>% group_by(PlotID, PrimaryKey, Species, ScientificName , CommonName , GrowthHabitSub, Duration) %>%
                                       dplyr::select(GrowthHabitSub , Duration , PlotID , PrimaryKey , Hgt_Species_Avg , 
                                        Hgt_Species_Avg_n)


Heights_Summary_Sagebrush <- HgtPrep_Sagebrush %>% group_by(Species, ScientificName , CommonName , GrowthHabitSub, Duration) %>%
                       summarize(AverageHeight_cm = mean(Hgt_Species_Avg) ,
                       Standard_Deviation_cm = sd(Hgt_Species_Avg),
                       MinHeight_cm = min(Hgt_Species_Avg) ,
                       MaxHeight_cm = max(Hgt_Species_Avg)) %>%
...
Heights_Summary_Sagebrush_PerPlot <- HgtPrep_Sagebrush %>% 
                                      group_by(PlotID, PrimaryKey, Species, ScientificName , CommonName , GrowthHabitSub, Duration) %>%     
                                      dplyr::select(GrowthHabitSub , Duration , PlotID , PrimaryKey , Hgt_Species_Avg , 
                                      Hgt_Species_Avg_n)
...
Height_Plot_Sagebrush <- ggplot(HgtPrep_Sagebrush, aes(x = Species , y = Hgt_Species_Avg , 
                                       text = paste("Plot Id: " , PlotID , 
                                      "PrimaryKey: " , PrimaryKey ,
                                       "Species: " , Species , 
                                       "Average Height (cm): "  , Hgt_Species_Avg , 
...
Height_Plot_Sagebrush_Static <- ggplot(HgtPrep_Sagebrush, aes(x = Species , y = Hgt_Species_Avg)) +
                        geom_boxplot() +
                        geom_jitter(width = .1 , shape = 21) + theme_light() +
                        theme(axis.ticks.y = element_blank() ,
                        axis.line.y = element_blank()) +  labs(x = "Species" , y = "Average Height, cm") + 
                        coord_flip() 
...
#SagebrushHeight_Ly <- ggplotly(Height_Plot_Sagebrush , tooltip = "text")
#SagebrushHeight_Ly
...
Heights_Summary_SFG_Plot %>%  DT::datatable(extensions = 'Buttons', filter = "top" , options = list(scrollX = TRUE ,
                          dom = 'Bfrtip',
                          buttons =
                          list(list(
                          extend = 'collection',
...
#Heights_Summary_Sagebrush_PerPlot %>%  DT::datatable(extensions = 'Buttons', filter = "top" , options = list(scrollX = TRUE ,
                          #dom = 'Bfrtip',
                          #buttons =
                          #list(list(
                          #extend = 'collection',
                          #buttons = c('csv', 'excel'),
                          #text = 'Download Table'))) , 
                          #caption = (paste("Heights by Sagebrush Species by plots within " , EcoSite)) ,
                          #rownames = FALSE)
```
...
#Heights_Summary_Sagebrush %>%  DT::datatable(extensions = 'Buttons', filter = "top" , options = list(scrollX = TRUE ,
                          #dom = 'Bfrtip',
                          #buttons =
                          #list(list(
                          #extend = 'collection',
                          #buttons = c('csv', 'excel'),
                          #text = 'Download Table'))) , 
                          #caption = (paste("Heights by Sagebrush Species in " , EcoSite)) ,
                          #rownames = FALSE)
```
