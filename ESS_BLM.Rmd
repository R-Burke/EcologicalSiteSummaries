...
author: "The Jornada, New Mexico State University" 
date: "10/15/2019"
output:
  html_document:
    theme: cosmo
...
*Type in the ecological site id between the " " in the code below (line 78).*

```{r file paths , echo = TRUE , include = TRUE}

#Define your ecological site
EcoSite <- c("R036XB006NM")
...
TDat_LMF_plots_Year <- TDat_LMF %>% filter(!is.na(EcologicalSiteId)) %>% filter(!is.na(DateEstablished)) %>% filter(!is.na(Latitude_NAD83)) %>%
...
# 3. Where is this data coming from? {.tabset .tabset-fade .tabset-pills}
##
## Plot Map \
...
Merged_Species_PercCover <- Merged_Species %>% subset(AH_SpeciesCover > 0.000000) %>% 
                                                ...
                                                select(-Hgt_Species_Avg , -Hgt_Species_Avg_n) 
...
SFG_Plots_Interactive <- lapply(X = split(Merged_Species, Merged_Species[["GrowthHabitSub"]] , drop = TRUE), FUN = function(Merged_Species){
  current_plot <- ggplot(Merged_Species , aes(x = GrowthHabitSub , y = AH_SpeciesCover, 
                                              text = paste("Plot ID: " , PlotID , "Species: " , 
                                                           ScientificName , "Code: " , Species , 
                                                           "Percent Cover: " , AH_SpeciesCover , "Noxious: " , Noxious , sep = "<br>"))) +
                                                                  geom_boxplot(width = .6 , outlier.shape = NA) +
                                                                  geom_jitter(width = .2 , size = 1.25) +
                                                                  theme_light() +
...
SFG_Plots_Static <- lapply(X = split(Species_plots_ecosite, Species_plots_ecosite[["GrowthHabitSub"]] , drop = TRUE), FUN = function(Species_plots_ecosite){
...
Species_Plots_Interactive <- lapply(X = split(Merged_Species, list(Merged_Species$GrowthHabitSub , Merged_Species$Duration) , drop = TRUE),
                                  FUN = function(Merged_Species){
   current_plot <- ggplot(Merged_Species , aes(x = Species , y = AH_SpeciesCover, text = paste("Plot ID: " , PlotID , "Species: " , 
                                                           ScientificName , "Code: " , Species , 
                                                           "Percent Cover: " , AH_SpeciesCover , "Noxious: " , Noxious , sep = "<br>"))) +
                                                           geom_boxplot(width = .6 , outlier.shape = NA) +
                                                           ...
                                                           theme_light() +
                                                           labs(y = "Percent Cover") + 
                                                           ggtitle(paste("Percent Cover by Species, " , Merged_Species$GrowthHabitSub)) + 
...
Species_Plots_Static <- lapply(X = split(Species_plots_ecosite, list(Species_plots_ecosite$GrowthHabitSub , Species_plots_ecosite$Duration) , 
                                         drop = TRUE),
     ...
     current_plot <- ggplot(Species_plots_ecosite , aes(x = Species , y = AH_SpeciesCover)) +
                            ...
                            theme_light() +
                            labs(y = "Percent Cover") + 
                            ggtitle(paste("Percent Cover by Species, " , Species_plots_ecosite$GrowthHabitSub)) + 
...
Merged_Species_SG_PrefForb <- Merged_Species_SG %>% mutate(PreferredForb = (SG.Group == "PreferredForb")) %>% subset(PreferredForb == TRUE) %>% 
                              subset(AH_SpeciesCover > 0.0000)
...
PrefForb_Plots_Interactive <-  if (nrow(Merged_Species_SG_PrefForb) < 1) {PrefForb_Plots_Interactive <- NULL} else{
                               ggplot(Merged_Species_SG_PrefForb , aes(x = Species , y = AH_SpeciesCover, 
                                                           ...
                                                           theme(axis.title.y = element_blank() , axis.title.x = element_blank()) +
                                                                 ...
                                                                 axis.title.y = element_blank()) +
                                                           coord_flip() 
                                   if(length(unique(Merged_Species_SG_PrefForb[["Duration"]])) > 1) {
                 PrefForb_Plots_Interactive <- facet_grid(rows = vars(Duration),
                                               switch = "y",
                                               scales = "free",
                                               drop = TRUE)}}
...
PrefForb_Static <- if (nrow(Merged_Species_SG_PrefForb) < 1) {PrefForb_Static <- NULL} else{
                      ggplot(Merged_Species_SG_PrefForb , aes(x = Species , y = AH_SpeciesCover)) +
                                                           ...
                                                           coord_flip() 
                                            if(length(unique(Merged_Species_SG_PrefForb[["Duration"]])) > 1) {
                  PrefForb_Static <- facet_grid(rows = vars(Duration),
                                     switch = "y",
                                     scales = "free",
                                     drop = TRUE)}}
...
SGGroup_Plots_Interactive <- ggplot(Merged_Species_SG , aes(x = SG.Group , y = AH_SpeciesCover, text = paste("Plot ID: " , PlotID , "Species: " , 
                                                           ScientificName , "Code: " , Species , 
                                                           "Sage Grouse Group: " , SG.Group , 
                                                           "Percent Cover: " , AH_SpeciesCover , "Noxious: " , Noxious , sep = "<br>"))) +
                                                           geom_boxplot(width = .6 , outlier.shape = NA) +
...
SageBrush_Plots_Interactive <-  if (nrow(SageBrushCover_Tall) < 1) {SageBrush_Plots_Interactive <- NULL} else{
                               ggplot(SageBrushCover_Tall , aes(x = SageBrushCover , y = Percent , 
                                                                text = paste("Plot ID: " , PlotID , "Primary Key: " , PrimaryKey , 
                                                                             sep = "<br>"))) +
                                                           geom_boxplot(width = .6 , outlier.shape = NA) +
                                                           geom_jitter(width = .15 , size = 1) +
                                                           scale_y_continuous(limits = c(0 , 100)) +
...
Nox_Non_Plot_Interactive <- ggplot(data = Nox_Non , (aes(x = Noxious , y = AH_SpeciesCover , text = paste("Plot ID: " , PlotID , "Species: " , 
                                                           ScientificName , "Code: " , Species , 
                                                           "Percent Cover: " , AH_SpeciesCover , 
                                                           "Noxious: " , Noxious , sep = "<br>")))) +
                                                 geom_boxplot(width = .6 , outlier.shape = NA) +
...
```{r species plots grass annual , fig.height = 5  , fig.width = 10 , echo = FALSE, include = TRUE , results = 'asis'}
...
```{r species plots grass annual static , fig.height = 8 , fig.width = 10 , echo = FALSE, include = TRUE , results = 'asis'}
...
```{r species plots grass perennial, fig.height = 5 , fig.width = 10 , echo = FALSE, include = TRUE , results = 'asis'}
...
```{r species plots grass perennial static, fig.height = 8 , fig.width = 10 , echo = FALSE, include = TRUE , results = 'asis'}
...
```{r species plots forb annual , fig.height = 5 , fig.width = 10 , echo = FALSE, include = TRUE , results = 'asis'}
...
```{r species plots forb annual static , fig.height = 8 , fig.width = 10 , echo = FALSE, include = TRUE , results = 'asis'}
...
```{r species plots forb perennial , fig.height = 5 , fig.width = 10 , echo = FALSE, include = TRUE , results = 'asis'}
...
```{r species plots forb perennial static, fig.height = 8 , fig.width = 10 , echo = FALSE, include = TRUE , results = 'asis'}
...
```{r species plots subshrub , fig.height = 5 , fig.width = 10 , echo = FALSE, include = TRUE , results = 'asis'}
...
```{r species plots subshrub static, fig.height = 8 , fig.width = 10 , echo = FALSE, include = TRUE , results = 'asis'}
...
```{r species plots shrub , fig.height = 5 , fig.width = 10 , echo = FALSE, include = TRUE , results = 'asis'}
...
```{r species plots shrub static , fig.height = 8 , fig.width = 10 , echo = FALSE, include = TRUE , results = 'asis'}
...
```{r species plots tree , fig.height = 5 , fig.width = 10 , echo = FALSE, include = TRUE , results = 'asis'}
...
```{r species plots tree static, fig.height = 8 , fig.width = 10 , echo = FALSE, include = TRUE , results = 'asis'}
...
<div class = "ScrollStyle">
```{r species plots succulent , fig.height = 5 , fig.width = 10 , echo = FALSE, include = TRUE , results = 'asis'}
...
<div class = "ScrollStyle">
```{r species plots succulent static , fig.height = 8 , fig.width = 10 , echo = FALSE, include = TRUE , results = 'asis'}
...
```{r sggroup interactive , fig.height = 5 , fig.width = 10 , echo = FALSE, include = TRUE , results = 'asis'}
...
```{r sggroup static , fig.height = 8 , fig.width = 10 , echo = FALSE, include = TRUE , results = 'asis'}
...
```{r pref forb interactive , fig.height = 5 , fig.width = 10 , echo = FALSE, include = TRUE , results = 'asis'}
...
<div class = "ScrollStyle">
```{r pref forb static, fig.height = 8 , fig.width = 10 , echo = FALSE, include = TRUE , results = 'asis'}
...
```{r sagebrush interactive , fig.height = 5 , fig.width = 10 , echo = FALSE, include = TRUE , results = 'asis'}
...
```{r pref sagebrush static, fig.height = 8 , fig.width = 10 , echo = FALSE, include = TRUE , results = 'asis'}
...
### Percent Cover by Species, Per Plot
Percent cover by species by plot within your specified ecological site. 
*Note that percent cover on a per-plot basis is not cumulative because we are calculating cover from any hit on line point intercept. Percent cover values per plot could therefore potentially add up to over 100. See Ground Cover Summary Table for total foliar cover estimates.*
...
### Percent Cover by Species, Ecological Site 
...
### Percent Cover, Noxious vs. Non-Noxious
...
### Percent Cover, Woody vs. Non-Woody 
...
### Percent Cover by Sage Grouse Group
...
### Percent Cover, Preferred Forbs

Percent cover by preferred forbs averaged across your specified ecological site. *Note: you can obtain these values on a per plot basis by filtering Preferred Forbs in the Percent Cover by Species per plot table.*
...
```{r LPI ground plot static , echo = FALSE , fig.height = 8 , fig.width = 10 , results = 'asis'}
...
Height_Plot_SFG <- ggplot(HgtPrep, aes(x = GrowthHabit , y = Hgt_Species_Avg , text = paste("Plot Id: " , PlotID , 
                                                                                            "PrimaryKey: " , PrimaryKey ,
                                                                                            "Species: " , Species , 
...
Height_Plot_SFG_Static <- ggplot(HgtPrep, aes(x = GrowthHabit , y = Hgt_Species_Avg)) +
                        geom_boxplot() +
                        geom_jitter(width = .1 , aes(text = PlotID)) +
                        labs(x = "Growth Habit" , y =  "Average Height, cm") + theme_light() +
                        theme(axis.text.y = element_blank() , axis.ticks.y = element_blank() ,
                        axis.line.y = element_blank()) + theme(axis.title.x = element_text(vjust=-4) ,
                                                          axis.title.y = element_text(hjust=-5)) +
...
```{r heights plot static , fig.height = 6 , fig.width = 10 , echo = FALSE , include = TRUE}
...
## Canopy Gap Size Class Summary Table
###
### Gap Classes, By Plot
